...
const request = (type, operationKey) => ({ resetType:type, operationKey, authorized:true, executionEnabled:true, confirmationPhrase:FactoryReset.CONFIRMATION_PHRASE, currentUsername:"owner", performedByUsername:"owner", performedBy:"QA Owner", backup, performedAt:"2026-08-02T12:00:00.000Z" });
...
test("التنفيذ مقفول افتراضيًا BLOCKED",()=>assert.throws(()=>FactoryReset.execute(fixture(),{...request("business","FLAG-OFF"),executionEnabled:false}),e=>e.code==="FACTORY_RESET_DISABLED"));
test("التنفيذ المفعّل بالـ Flag يسمح بعد اكتمال الشروط",()=>{const result=FactoryReset.execute(fixture(),request("business","FLAG-ON"));assert.strictEqual(result.validation.valid,true);});
...
test("Wrong username BLOCKED",()=>assert.throws(()=>FactoryReset.execute(fixture(),{...request("business","BAD-USER"),currentUsername:"manager"}),e=>e.code==="IDENTITY_CONFIRMATION_FAILED"));
test("التأكيد الثاني مطلوب",()=>assert.throws(()=>FactoryReset.assertConfirmedExecution({},request("business","NO-SECOND"),20_000),e=>e.code==="SECOND_CONFIRMATION_REQUIRED"));
test("انتظار 10 ثوانٍ مفروض من الخادم",()=>{const req=request("factory","WAIT-10"),prepared=FactoryReset.confirmExecution(fixture(),{backup},req,20_000);assert.throws(()=>FactoryReset.assertConfirmedExecution(prepared,req,29_999),e=>e.code==="CONFIRMATION_DELAY_ACTIVE");assert.strictEqual(FactoryReset.assertConfirmedExecution(prepared,req,30_000),true);});
