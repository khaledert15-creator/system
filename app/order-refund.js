(function (root, factory) {
  ...
  function customerLedger(db, customerId) {
    const rows=[];
    for(const sale of list(db,"sales").filter(x=>x.customerId===customerId&&!x.deletedAt))rows.push({date:sale.createdAt||sale.date,type:"invoice",reference:sale.id,description:`فاتورة مبيعات${sale.onlineOrderId?` — ${sale.onlineOrderId}`:""} · خصم ${round(sale.discount||0)} · شحن ${round(sale.shippingFee??sale.shipping??sale.shippingCost??0)}`,debit:sale.status==="ملغاة"?0:round(sale.total),credit:0,status:sale.status||"معتمدة",user:sale.createdByName||sale.createdByUsername||"",note:sale.notes||sale.note||"",links:{invoiceId:sale.id,orderId:sale.onlineOrderId||""}});
