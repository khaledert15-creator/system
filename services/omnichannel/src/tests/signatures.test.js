...
test("verifies Meta style sha256 signatures", () => {
  ...
  assert.equal(verifyMetaSignature({ appSecret: secret, rawBody, signature, required: true }), true);
  assert.equal(verifyMetaSignature({ appSecret: secret, rawBody, signature: "sha256=bad", required: true }), false);
});
